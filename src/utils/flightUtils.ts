...
export function projectFlightPosition<T extends {
  longitude: number;
  latitude: number;
  altitude: number;
  velocity: number;
  ...
  onGround: boolean;
  updatedAt?: number;
}>(flight: T, now: number): T {
  if (flight.onGround || flight.velocity < 12 || !flight.updatedAt) return flight;

  const ageSeconds = Math.max(0, Math.min(22, (now - flight.updatedAt) / 1000));
  if (ageSeconds <= 0.1) return flight;

  const distanceMeters = flight.velocity * ageSeconds;
  ...
  const nextLat = Math.max(-85, Math.min(85, flight.latitude + (Math.cos(headingRad) * distanceMeters) / metersPerDegreeLat));
  const nextLon = normalizeLongitude(flight.longitude + (Math.sin(headingRad) * distanceMeters) / metersPerDegreeLon);
  const nextAlt = Math.max(0, flight.altitude + flight.verticalRate * ageSeconds);
...
export function projectLngLat(
  ...
  return [
    normalizeLongitude(longitude + (Math.sin(headingRad) * distanceMeters) / metersPerDegreeLon),
    Math.max(-85, Math.min(85, latitude + (Math.cos(headingRad) * distanceMeters) / metersPerDegreeLat)),
  ];
}
